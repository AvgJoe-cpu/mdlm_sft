...
if __name__ == "__main__":
    ...
    # ── Smoke test: `python -m ... evaluate_mdlm --smoke_test` ───────────────
    # Builds a tiny fake dataset, runs eval end-to-end, verifies outputs exist.
    if "--smoke_test" in sys.argv:
        ...
        log.info("Running smoke test with fake data.")

        rng = np.random.default_rng(0)
        n = 8
        fake = Dataset.from_dict({
            "id":         [f"ex_{i}" for i in range(n)],
            "prompt":     [f"Prompt number {i}: what is {i}+{i}?" for i in range(n)],
            "completion": [f"The answer is {2*i}." for i in range(n)],
        })

        with tempfile.TemporaryDirectory() as tmp:
            gold_path   = Path(tmp) / "fake_ds"
            out_path    = Path(tmp) / "eval_out"
            fake.save_to_disk(str(gold_path))

            cfg = MDLMEvalConfig(
                model_name_or_path         = "bert-base-uncased",
                gold_path                  = str(gold_path),
                save_summary_path          = str(out_path),
                max_length                 = 64,
                per_device_eval_batch_size = 4,
                dataloader_num_workers     = 0,
                bf16                       = False,   # portable: works on CPU/MPS
                num_mc_samples             = 1,
                time_epsilon               = 1e-3,
                eval_seed                  = 0,
            )
            log.info("Smoke-test config:\n%s", json.dumps(cfg.__dict__, indent=2, default=str))

            run_evaluation(cfg)

            produced = list(out_path.glob("*"))
            assert any(p.name.endswith("_corpus.json")      for p in produced), f"missing corpus json in {produced}"
            assert any(p.name.endswith("_per_example.jsonl") for p in produced), f"missing per-example jsonl in {produced}"
            corpus_file = next(p for p in produced if p.name.endswith("_corpus.json"))
            log.info("Smoke test OK. Corpus:\n%s", corpus_file.read_text())

...
BASE_MODEL_ID     = "avgJo3/mdlm-sft-M2-100pct-1ep"   # ← hardcoded HF repo id
...
def base_rounds(n_rounds: int) -> dict:
    """BASE: each round reloads the base model, trains on previous round's generation."""
    rounds = {}
...
def mix_rounds(n_rounds: int, mix_factor: float = 1.0) -> dict:
    ...
    then trains the base model on the mix. Reuses BASE's R0 generation for R1."""
    rounds = {}
